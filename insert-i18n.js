...
const en = {
  ...
  deliveryNoteAuto: 'Download link shown immediately after verification (usually <3 sec)',
...
const i18nObj = `\n    var i18n = {\n      zh: {}, // Chinese is default (hardcoded in HTML)\n      en: ${JSON.stringify(en, null, 8).replace(/\n/g, '\n      ')}\n    };\n  `;

// 插入到第一个 <script> 标签内的开头部分
const insertPos = html.indexOf('<script>') + '<script>'.length;
if (insertPos > 0) {
  html = html.slice(0, insertPos) + i18nObj + html.slice(insertPos);
  fs.writeFileSync('index.html', html, 'utf8');
  console.log('✅ i18n object inserted into index.html');
  console.log('📊 Total English translations:', Object.keys(en).length);
} else {
